...
- API keys are optional for local dev. With `GROQ_API_KEY` / `groqCloudKey` unset, `POST /api/chat` returns a stubbed reply (see `server/src/routes/chat.js`), so the game and chat UI are fully exercisable end-to-end without any secrets. Do NOT put placeholder values like `your_key_here` in `.env` — any non-empty Groq key forces a real Groq call that will fail. To enable real AI chat, set a valid `GROQ_API_KEY` (or Cursor Cloud secret `groqCloudKey`); for voice, set `ELEVENLABS_API_KEY` (or Cursor Cloud secret `elevenLabsKey`) — `/api/tts` returns HTTP 501 without it.
